...
def pyramid_down(image, device="cpu"):
    ...
    multiband = [F.conv2d(image[:, i:i + 1,:,:], gaussian_k, padding=2, stride=2) for i in range(1)]
...
def pyramid_up(image, device="cpu"):
    ...
    multiband = [F.conv2d(upsample[:, i:i + 1,:,:], gaussian_k, padding=2) for i in range(1)]
...
def extract_patches(pyramid_layer, slice_indices,
    ...
    x = (x - mean) / (std + 1e-8)
    # reshape to 2rank
    x = x.reshape(-1, slice_size * slice_size)
    return x
