...
class ptychographicData(Dataset):
	...
	def __getitem__(self, idx):
		...
		image /= np.max(image) #rescale to 0..1
		if self.transform:
	...
	def scaleUp(self, row):
		return self.scalingFactors*np.array(row)
...
with torch.no_grad(), open('resultsTest_{}.csv'.format(args["model"]), 'w+', newline='') as resultsTest:
